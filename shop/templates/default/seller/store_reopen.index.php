...
<div class="alert alert-block mt10">
  <ul>
    <li>1. 煤企到期前 30 天可以申请煤企续签。</li>
    <?php if (empty($output['reopen_list'])) { ?>
    <li>2. 您的煤企已签约至 <?php echo date('Y-m-d',$output['store_info']['store_end_time']);?>，自 <?php echo date('Y-m-d',$output['store_info']['allow_applay_date']);?> 起的 30 天内可以申请续签。</li>
    <?php } ?>
  </ul>
</div>
...
<?php if ($output['applay_reopen']) {?>
  <form method="post" action="index.php?act=store_info&op=reopen_add" name="add_form" id="add_form">
    <input type="hidden" name="form_submit" value="ok" />
    <dl>
      <dt>申请煤企等级<?php echo $lang['nc_colon'];?></dt>
      <dd>
