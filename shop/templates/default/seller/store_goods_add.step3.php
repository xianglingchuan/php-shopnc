...
<form method="post" id="goods_image" action="<?php if ($output['edit_goods_sign']) { echo urlShop('store_goods_online', 'edit_save_image'); } else { echo urlShop('store_goods_add', 'save_image');}?>">
  ...
  <div class="ncsc-form-goods-pic">
    ...
    <li>3. 每种颜色最多可上传5张图片或从图片空间中选择已有的图片，上传后的图片也将被保存在煤企图片空间中以便其它使用。</li>
    <li>4. 通过更改排序数字修改煤炭图片的排列显示顺序。</li>
  ...
  <?php }?>
  <div class="bottom tc hr32"><label class="submit-border"><input type="submit" class="submit" value="<?php if ($output['edit_goods_sign']) { echo '提交'; } else { ?><?php echo $lang['store_goods_add_next'];?>，确认煤炭发布<?php }?>" /></label></div>
</form>
