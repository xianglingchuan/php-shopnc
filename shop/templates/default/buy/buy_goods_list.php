...
<div class="ncc-receipt-info">
  ...
  <table class="ncc-table-style">
    ...
    <tbody>
      <tr>
        <th colspan="20"><strong>煤企：<a href="<?php echo urlShop('show_store','index',array('store_id'=>$store_id));?>"><?php echo $cart_list[0]['store_name']; ?></a></strong> <span member_id="<?php echo $output['store_list'][$store_id]['member_id'];?>"></span>
          <div class="store-sale">
            <?php if (!empty($output['cancel_calc_sid_list'][$store_id])) {?>
            <em><i class="icon-gift"></i>煤企活动-免运费</em><?php echo $output['cancel_calc_sid_list'][$store_id]['desc'];?>
            <?php } ?>
            <?php if (!empty($output['store_mansong_rule_list'][$store_id])) {?>
            <em><i class="icon-gift"></i>煤企活动-满即送</em><?php echo $output['store_mansong_rule_list'][$store_id]['desc'];?>
      ...
      <tr id="cart_item_<?php echo $cart_info['cart_id'];?>" class="shop-list <?php echo ($cart_info['state'] && $cart_info['storage_state']) ? '' : 'item_disabled';?>">
        ...
        <td class="tl" <?php if ($cart_info['bl_id'] != '0') {?>colspan="2"<?php }?>><dl class="ncc-goods-info">
            ...
            <dd> <span class="groupbuy">团购</span></dd>
            <?php }?>
            <?php if ($cart_info['bl_id'] != '0') {?>
