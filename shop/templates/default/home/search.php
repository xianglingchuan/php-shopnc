...
<div class="nch-container wrapper">
    <div class="left">
        <?php if (!empty($output['goods_class_array'])) { ?>
            <!-- S 隐藏分类筛选-->
            <div class="nch-module nch-module-style02" style="display: none">
                <div class="title">
                    <h3>分类筛选</h3>
                </div>
                <div class="content">
                    <ul id="files" class="tree">
                        <?php foreach ($output['goods_class_array'] as $value) { ?>
                            <li><i class="tree-parent tree-parent-collapsed"></i><a
                                    href="<?php echo urlShop('search', 'index', array('cate_id' => $value['gc_id'], 'keyword' => $_GET['keyword'])); ?>"
                                    <?php if ($value['gc_id'] == $_GET['cate_id']) { ?>class="selected"<?php } ?>><?php echo $value['gc_name'] ?></a>
                                <?php if (!empty($value['class2'])) { ?>
                                    <ul>
                                        <?php foreach ($value['class2'] as $val) { ?>
                                            <li><i class="tree-parent tree-parent-collapsed"></i><a
                                                    href="<?php echo urlShop('search', 'index', array('cate_id' => $val['gc_id'], 'keyword' => $_GET['keyword'])); ?>"
                                                    <?php if ($val['gc_id'] == $_GET['cate_id']) { ?>class="selected"<?php } ?>><?php echo $val['gc_name'] ?></a>
                                                <?php if (!empty($val['class3'])) { ?>
                                                    <ul>
                                                        <?php foreach ($val['class3'] as $v) { ?>
                                                            <li class="tree-parent tree-parent-collapsed"><i></i><a
                                                                    href="<?php echo urlShop('search', 'index', array('cate_id' => $v['gc_id'], 'keyword' => $_GET['keyword'])); ?>"
                                                                    <?php if ($v['gc_id'] == $_GET['cate_id']) { ?>class="selected"<?php } ?>><?php echo $v['gc_name'] ?></a>
                                                            </li>
                                                        <?php } ?>
                                                    </ul>
                                                <?php } ?>
...
<!--        <div class="nch-module nch-module-style03">
    ...
    <div class="right" style="width:100%">
        <?php if (!isset($output['goods_class_array']['child']) && empty($output['goods_class_array']['child']) && !empty($output['goods_class_array'])) { ?>
            ...
            <?php if ((!empty($output['brand_array']) && is_array($output['brand_array'])) || (!empty($output['attr_array']) && is_array($output['attr_array']))) { ?>
                <div class="nch-module " style="position: relative; border:none;">
                    <div class="title"
                         style="border-top:solid 2px #BE000E;border-left:1px solid #d7d7d7;border-right: 1px solid #d7d7d7;">
                        <h3>
                    ...
                    <div class="content" style="border-left:1px solid #d7d7d7;border-right: 1px solid #d7d7d7;">
                        <div class="nch-module-filter">
                                    ...
                                    <dd class="list">
                                        <?php if (isset($output['checked_brand']) && is_array($output['checked_brand'])) { ?>
                                            <?php foreach ($output['checked_brand'] as $key => $val) { ?>
                                                <span class="selected"
                                                      nctype="span_filter"><?php echo $lang['goods_class_index_brand']; ?>
                                                    :<em><?php echo $val['brand_name'] ?></em><i
                                                        data-uri="<?php echo removeParam(array('b_id' => $key)); ?>">X</i></span>
                                            <?php } ?>
                                        <?php } ?>
                                        <?php if (isset($output['checked_attr']) && is_array($output['checked_attr'])) { ?>
                                            <?php foreach ($output['checked_attr'] as $val) { ?>
                                                <span class="selected"
                                                      nctype="span_filter"><?php echo $val['attr_name'] . ':<em>' . $val['attr_value_name'] . '</em>' ?>
                                                    <i data-uri="<?php echo removeParam(array('a_id' => $val['attr_value_id'])); ?>">X</i></span>
                                            <?php } ?>
                                        <?php } ?>
                                    </dd>
                                </dl>
                                        ...
                                        <dd class="list">
                                            <ul>
                                                ...
                                                foreach ($output['brand_array'] as $k => $v) {
                                                    $i++; ?>
                                                    <li <?php if ($i > 10){ ?>style="display:none"
                                                        nc_type="none"<?php } ?>><a
                                                            href="<?php $b_id = (($_GET['b_id'] != '' && intval($_GET['b_id']) != 0) ? $_GET['b_id'] . '_' . $k : $k);
                                                            echo replaceParam(array('b_id' => $b_id)); ?>"><?php echo $v['brand_name']; ?></a>
                                                    </li>
                                                <?php } ?>
                                            </ul>
                                        </dd>
                                        <?php if (count($output['brand_array']) > 10) { ?>
                                            <dd class="all"><span nc_type="show"><i
                                                        class="icon-angle-down"></i><?php echo $lang['goods_class_index_more']; ?></span>
                                            </dd>
                                        <?php } ?>
                                    </dl>
                                    <?php $dl++;
                            ...
                            <dl>
                                <?php } else { ?>
                                <dl class="trigger_item" style="display:none">
                                    <?php } ?>
                                    ...
                                    <dd class="list">
                                        <ul>
                                            ...
                                            foreach ($val['value'] as $k => $v) {
                                                ...
                                                <li <?php if ($i > 10){ ?>style="display:none"
                                                    nc_type="none"<?php } ?>><a
                                                        href="<?php $a_id = (($_GET['a_id'] != '' && $_GET['a_id'] != 0) ? $_GET['a_id'] . '_' . $k : $k);
                                                        echo replaceParam(array('a_id' => $a_id)); ?>"><?php echo $v['attr_value_name']; ?></a>
                                                </li>
                                            <?php } ?>
                                        </ul>
                                    </dd>
                    ...
                    <div id="advanced" style="position: relative;
    overflow: visible;
    height: 0;
    ...
    text-align: center;
    font-size: 0;">
                        <div class="attr-extra" style="  position: relative;
    ...
    cursor: pointer;
    font-size: 12px;">
                            <div class="more_button" style="position: relative; height: 24px;
    padding: 0 32px 0 24px;
    line-height: 24px;"><i class="icon-angle-down" style="margin-right:4px;"></i>更多选项
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="shop_con_list" id="main-nav-holder">
            <nav class="sort-bar" id="main-nav">
                ...
                <div class="nch-sortbar-owner"><span><a href="<?php if ($_GET['type'] == 1) {
                            echo dropParam(array('type'));
                        } else {
                            echo replaceParam(array('type' => '1'));
                        } ?>" <?php if ($_GET['type'] == 1) { ?>class="selected"<?php } ?>><i></i>平台自营</a></span></div>
                <div class="nch-sortbar-owner"><span><a href="<?php if ($_GET['gift'] == 1) {
                            echo dropParam(array('gift'));
                        } else {
                            echo replaceParam(array('gift' => '1'));
                        } ?>" <?php if ($_GET['gift'] == 1) { ?>class="selected"<?php } ?>><i></i>赠品</a></span></div>
                <div class="nch-sortbar-location">煤炭所在地：
                    <div class="select-layer">
                        <div class="holder"><em nc_type="area_name">
                                <?php echo $lang['goods_class_index_area']; ?><!-- 所在地 --></em></div>
                        <div class="selected"><a nc_type="area_name">
                                <?php echo $lang['goods_class_index_area']; ?><!-- 所在地 --></a></div>
                        <i class="direction"></i>
                        <ul class="options">
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $('.more_button').click(function () {
            if (isExpand) {
                $('.trigger_item').hide();
                $(this).html('<i class="icon-angle-down" style="margin-right:4px;"></i>更多选项');
            } else {
                $('.trigger_item').show();
                $(this).html('<i class="icon-angle-up"  style="margin-right:4px;"></i>收起');
            }
            isExpand = !isExpand;
        });
