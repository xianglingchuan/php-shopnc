...
class store_goods_onlineControl extends BaseSellerControl {
    ...
    public function edit_goodsOp() {
        ...
        // 是否能使用编辑器
        if(checkPlatformStore()){ // 平台煤企可以使用编辑器
    ...
    public function edit_save_goodsOp() {
        ...
        $update_common['goods_state']        = ($this->store_info['store_state'] != 1) ? 0 : intval($_POST['g_state']);            // 煤企关闭时，煤炭下架
        ...
        $update_common['is_appoint']         = intval($_POST['is_appoint']);     // 只有库存为零的煤炭可以预约
        $update_common['appoint_satedate']   = $update_common['is_appoint'] == 1 ? strtotime($_POST['g_saledate']) : '';   // 预约煤炭的销售时间
        $update_common['is_presell']         = $update_common['goods_state'] == 1 ? intval($_POST['is_presell']) : 0;     // 只有出售中的煤炭可以预售
        $update_common['presell_deliverdate']= $update_common['is_presell'] == 1? strtotime($_POST['g_deliverdate']) : ''; // 预售煤炭的发货时间
    ...
    public function drop_goodsOp() {
        ...
        if ($return) {
            // 添加操作日志
            $this->recordSellerLog('删除煤炭，平台货号：'.$common_id);
    ...
    public function goods_unshowOp() {
        ...
        if ($return) {
            ...
            // 添加操作日志
            $this->recordSellerLog('煤炭下架，平台货号：'.$common_id);
    ...
    public function save_giftOp() {
        ...
        foreach ($data as $key => $val) {
            ...
            $owner_gid = intval($key);  // 主煤炭id
            // 验证主煤炭是否为本煤企煤炭,如果不是本店煤炭继续下一个循环
        ...
        if (!empty($data)) {
            foreach ($data as $key => $val) {
                ...
                foreach ($val as $v) {
                    ...
                    // 验证推荐组合煤炭是否为本煤企煤炭，如果不是本店煤炭继续下一个循环
                    $combo_info = $model_goods->getGoodsInfoByID($combo_gid, 'store_id,is_virtual,is_fcode,is_presell');
                    $is_general = $model_goods->checkIsGeneral($combo_info);     // 验证是否为普通煤炭
    ...
    private function profile_menu($menu_type,$menu_key, $allow_promotion = array()) {
        ...
        switch ($menu_type) {
            case 'goods_list':
                $menu_array = array(
                   array('menu_key' => 'goods_list',    'menu_name' => '出售中的煤炭', 'menu_url' => urlShop('store_goods_online', 'index'))
                );
                break;
            case 'edit_detail':
                if ($allow_promotion['lock'] === false) {
                    $menu_array = array(
                        array('menu_key' => 'edit_detail',  'menu_name' => '编辑煤炭', 'menu_url' => urlShop('store_goods_online', 'edit_goods', array('commonid' => $_GET['commonid'], 'ref_url' => $_GET['ref_url']))),
