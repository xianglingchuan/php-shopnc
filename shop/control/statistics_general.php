...
class statistics_generalControl extends BaseSellerControl {
	...
	public function generalOp(){
		...
		if (!checkPlatformStore()) {//如果不是平台煤企，则查询煤企经营类目的同行数据
    		//查询煤企经营类目
	...
	/*private function profile_menu($menu_key='') {
        $menu_array	= array(
            1=>array('menu_key'=>'stat_general','menu_name'=>'煤企概况',	'menu_url'=>'index.php?act=statistics_general&op=general')
        );
    ...
    private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'setting':
				$menu_array = array(
				    1=>array('menu_key'=>'pricesetting','menu_name'=>'煤炭价格区间',	'menu_url'=>'index.php?act=statistics_general&op=pricesetting'),
