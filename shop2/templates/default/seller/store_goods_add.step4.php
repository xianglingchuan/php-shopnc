...
<div class="alert alert-block hr32">
  <h2><i class="icon-ok-circle mr10"></i><?php echo $lang['store_goods_step3_goods_release_success'];?>&nbsp;&nbsp;<?php if (C('goods_verify')) {?>等待管理员审核煤炭！<?php }?></h2>
  <div class="hr16"></div>
  <div>
    <?php if ($output['allow_gift']) {?>
    <a class="ncsc-btn ncsc-btn-green ml30" href="<?php echo urlShop('store_goods_online', 'add_gift', array('commonid' => $_GET['commonid'], 'ref_url' => urlShop('store_goods_online', 'index')));?>"><i class="icon-gift"></i>为该煤炭添加赠品捆绑</a>
    <?php }?>
    <?php if ($output['allow_combo']) {?>
    <a class="ncsc-btn ncsc-btn-orange ml10" href="<?php echo urlShop('store_goods_online', 'add_combo', array('commonid' => $_GET['commonid'], 'ref_url' => urlShop('store_goods_online', 'index')));?>"><i class="icon-thumbs-up "></i>为该煤炭添加推荐组合</a></div>  
    <?php }?>
  <div class="hr16"></div>
  <strong>
