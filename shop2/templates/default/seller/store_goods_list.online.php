...
<table class="ncsc-default-table">
  ...
  <tbody>
    ...
    <tr>
      <td class="trigger"><i class="tip icon-plus-sign" nctype="ajaxGoodsList" data-comminid="<?php echo $val['goods_commonid'];?>" title="点击展开查看此煤炭全部规格；规格值过多时请横向拖动区域内的滚动条进行浏览。"></i></td>
      ...
      <td class="tl"><dl class="goods-name">
          <dt style="max-width: 450px !important;">
            ...
            <span class="type-virtual" title="虚拟兑换煤炭">虚拟</span>
            <?php }?>
            <?php if ($val['is_fcode'] ==1) {?>
            <span class="type-fcode" title="F码优先购买煤炭">F码</span>
            <?php }?>
            <?php if ($val['is_presell'] ==1) {?>
            <span class="type-presell" title="预先发售煤炭">预售</span>
            <?php }?>
            <?php if ($val['is_appoint'] ==1) {?>
            <span class="type-appoint" title="预约销售提示煤炭">预约</span>
            <?php }?>
          ...
          <dd class="serve"> <span class="<?php if ($val['goods_commend'] == 1) { echo 'open';}?>" title="店铺推荐煤炭"><i class="commend">荐</i></span> <span class="<?php if ($val['mobile_body'] != '') { echo 'open';}?>" title="手机端煤炭详情"><i class="icon-tablet"></i></span> <span class="" title="煤炭页面二维码"><i class="icon-qrcode"></i>
      ...
      <td class="nscs-table-handle"><?php if ($val['goods_lock'] == 0) {?>
        ...
        <?php } else {?>
        <span class="tip" title="该煤炭参加团购活动期间不能进行编辑及删除等操作,可以编辑赠品和推荐组合"><a href="<?php if ($val['is_virtual'] ==1 ) {echo 'javascript:void(0);';} else {echo urlShop('store_goods_online', 'add_gift', array('commonid' => $val['goods_commonid']));}?>" class="btn-orange-current"><i class="icon-lock"></i>
        <p>锁定</p>
        </a></span>
        <?php }?></td>
    </tr>
