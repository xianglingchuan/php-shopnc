  ...
  <div class="ncg-layout-l">
    ...
    <div class="ncg-title-bar">
      <ul class="tabs-nav">
        ...
        <li><a href="javascript:void(0);">煤炭评价(<?php echo $output['evaluate_info']['all'];?>)</a></li>
      </ul>
    </div>
...
<?php } ?>
          ...
          <li>2. 消费团购兑换码/券时，请向企业提供系统发送的“虚拟团购兑换码”，一码一销。</li>
<?php if ($output['buy_limit'] > 0) { ?>
      ...
      <div class="ncg-evaluate">
        <div class="top">
          ...
          <div class="btns"><span>您可对已购煤炭进行评价</span>
            <p><a href="<?php if ($output['groupbuy_info']['is_vr']) { echo urlShop('member_vr_order', 'index');} else { echo urlShop('member_order', 'index');}?>" class="ncg-btn ncg-btn-red" target="_blank"><i class="icon-comment-alt"></i>评价</a></p>
          </div>
        </div>
        <!-- 煤炭评价内容部分 -->
...
<?php if (!$output['store_info']['is_own_shop']) { ?>
          ...
          <div class="detail-rate">
            <h5><strong>企业动态评分</strong>与行业相比</h5>
            <ul class="rate">
...
<?php } ?>
          ...
          <div class="goto"> <a href="<?php echo urlShop('show_store', 'index', array('store_id' => $output['groupbuy_info']['store_id']));?>" >进入该企业</a></div>
        </div>
      </div>
    </div>
<?php } ?>
