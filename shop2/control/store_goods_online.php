...
class store_goods_onlineControl extends BaseSellerControl {
    ...
    public function edit_save_goodsOp() {
        ...
        $update_common['goods_state']        = ($this->store_info['store_state'] != 1) ? 0 : intval($_POST['g_state']);            // 店铺关闭时，煤炭下架
        ...
        if (!empty($data)) {
            foreach ($data as $key => $val) {
                ...
                foreach ($val as $v) {
                    ...
                    // 验证推荐组合煤炭是否为本店铺煤炭，如果不是本店煤炭继续下一个循环
