...
<div class="ncm-flow-layout">
  <div class="ncm-flow-container">
    ...
    <div class="alert">
      <h4>操作提示：</h4>
      <ul>
        <li>1. 若您未收到货，或已收到货且与企业达成一致不退货仅退款时，请选择<em>“仅退款”</em>选项。</li>
        <li>2. 若为煤炭问题，或者不想要了且与企业达成一致退货，请选择<em>“退货退款”</em>选项，退货后请保留物流底单。</li>
        <li>3. 若提出申请后，企业拒绝退款或退货，可再次提交申请或选择<em>“煤炭投诉”</em>，请求平台客服人员介入。</li>
        <li>4. 成功完成退款/退货；经过平台审核后，会将退款金额以<em>“预存款”</em>的形式返还到您的余额账户中（充值卡部分只能退回到充值卡余额）。</li>
      </ul>
    </div>
      ...
      <div class="ncm-default-form">
        <form id="post_form1" enctype="multipart/form-data" method="post" action="index.php?act=member_refund&op=add_refund&order_id=<?php echo $output['order']['order_id']; ?>&goods_id=<?php echo $output['goods']['rec_id']; ?>">
          ...
          <dl>
            ...
            <dd>
              <input type="text" class="text w50" name="refund_amount" value="<?php echo $output['goods']['goods_pay_price']; ?>" /><em class="add-on"><i class="icon-renminbi"></i></em>（最多 <strong class="green" title="可退金额由系统根据订单煤炭实际成交额和已退款金额自动计算得出"><?php echo $output['goods']['goods_pay_price']; ?></strong> 元） <span class="error"></span>
        ...
        <form id="post_form2" method="post" enctype="multipart/form-data" action="index.php?act=member_refund&op=add_refund&order_id=<?php echo $output['order']['order_id']; ?>&goods_id=<?php echo $output['goods']['rec_id']; ?>">
          ...
          <dl>
            ...
            <dd>
              ...
              <em class="add-on"><i class="icon-renminbi"></i></em> （最多 <strong class="green" title="可退金额由系统根据订单煤炭实际成交额和已退款金额自动计算得出。"><?php echo $output['goods']['goods_pay_price']; ?></strong> 元） <span class="error"></span>
              <p class="hint"><?php echo '退款金额不能超过可退金额。';?></p>
            </dd>
          </dl>
          <dl>
