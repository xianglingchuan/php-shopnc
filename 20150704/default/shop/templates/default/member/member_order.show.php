...
<div class="ncm-oredr-show">
  <div class="ncm-order-info">
    ...
    <div class="ncm-order-condition">
      ...
      <ul>
        <li>1. 您尚未对该订单进行支付，请<a href="index.php?act=buy&op=pay&pay_sn=<?php echo $output['order_info']['pay_sn'];?>" class="ncm-btn-mini ncm-btn-orange"><i></i>支付订单</a>以确保企业及时发货。</li>
        <li>2. 如果您不想购买此订单的煤炭，请选择<a href="#order-step" class="ncm-btn-mini">取消订单</a>操作。</li>
    ...
    <div class="ncm-order-condition">
      ...
      <ul>
        ...
        <li>2. 订单已提交企业进行备货发货准备。</li>
        <li>3. 如果您不想购买此订单的煤炭，请选择<a href="#order-step" class="ncm-btn-mini">取消订单</a>操作。</li>
        <?php } else { ?>
        <li>1. 您已使用“<?php echo orderPaymentName($output['order_info']['payment_code']);?>”方式成功对订单进行支付，支付单号 “<?php echo $output['order_info']['pay_sn'];?>”。</li>
        <li>2. 订单已提交企业进行备货发货准备。</li>
        <li>3. 如果您想取消购买，请与企业沟通后对订单进行<a class="ncm-btn-mini" href="#order-step">申请退款</a>操作。</li>
        <?php } ?>
      </ul>
    </div>
    <?php } ?>
    ...
    <div class="ncm-order-condition">
      ...
      <ul>
        ...
        <li>2. 如果您已收到货，且对煤炭满意，您可以<a href="#order-step" class="ncm-btn-mini ncm-btn-green">确认收货</a>完成交易。</li>
        <li>3. 系统将于
    ...
    <div class="ncm-order-condition">
      ...
      <ul>
        <li>1. 您已对该笔订单进行了煤炭及交易评价，感谢您的支持，祝您购物愉快！</li>
        <li>2. 将感兴趣的<a href="index.php?act=member_favorites&op=fglist" class="ncm-btn-mini">收藏煤炭</a>加入采购平台进行购买。</li>
        <li>3. 看一看<a href="<?php echo urlShop('show_store','index',array('store_id'=>$output['order_info']['store_id']), $output['order_info']['extend_store']['store_domain']);?>" class="ncm-btn-mini">该煤企</a>中有什么新煤炭上架。</li>
      </ul>
    </div>
    <?php } else { ?>
    <div class="ncm-order-condition">
      ...
      <ul>
        <li>1. 如果收到货后出现问题，您可以联系企业协商解决。</li>
        <li>2. 如果企业没有履行应尽的承诺，您可以申请 <a href="#order-step" class="red">"投诉维权"</a>。</li>
        <?php if ($output['order_info']['if_evaluation']) { ?>
        <li>3. 交易已完成，你可以对购买的煤炭及企业的服务进行<a href="#order-step" class="ncm-btn-mini ncm-btn-acidblue">评价</a>及晒单。</li>
        <?php } ?>
      </ul>
    </div>
    <?php } ?>
      ...
      <tbody>
        ...
        <tr class="bd-line">
          ...
          <td rowspan="<?php echo $output['order_info']['goods_count'];?>"><?php if ($output['order_info']['if_lock']) { ?>
            ...
            <p><a href="javascript:void(0)" nc_type="sharegoods" data-param='{"gid":"<?php echo $output['order_info']['extend_order_goods'][0]['goods_id'];?>"}'>分享煤炭</a></p>
            <?php } ?></td>
          <?php } ?>
          <!-- E 合并TD --> 
        </tr>
