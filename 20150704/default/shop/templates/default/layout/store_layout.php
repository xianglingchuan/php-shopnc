...
<div class="banner"><a href="<?php echo urlShop('show_store', 'index', array('store_id'=>$output['store_info']['store_id']));?>" class="img">
    ...
    <div id="nav" class="ncs-nav">
      <ul>
        ...
        <li class="<?php if ($output['page'] == 'store_sns') {?>active<?php }else{?>normal<?php }?>"><a href="<?php echo urlShop('store_snshome', 'index', array('sid' => $output['store_info']['store_id']))?>"><span>煤企动态<i></i></span></a></li>
...
$(function(){
	...
	$('a[nctype="share_store"]').click(function(){
		...
		login_dialog();
		<?php } else {?>
		ajax_form('sharestore', '分享煤企', 'index.php?act=member_snsindex&op=sharestore_one&inajax=1&sid=<?php echo $output['store_info']['store_id'];?>');
		<?php }?>
	});
});
</script>
