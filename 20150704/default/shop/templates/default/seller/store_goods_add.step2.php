...
<h3>页面导航</h3>
  <ul>
    ...
    <!--<li><a id="demo3Btn" href="#demo3" class="demoBtn">特殊煤炭</a></li>-->
  ...
  <form method="post" id="goods_form" action="<?php if ($output['edit_goods_sign']) { echo urlShop('store_goods_online', 'edit_save_goods');} else { echo urlShop('store_goods_add', 'save_goods');}?>">
    ...
    <div class="ncsc-form-goods">
      ...
      <dl>
        ...
        <dd nc_type="no_spec">
          ...
          <p class="hint"><?php echo $lang['store_goods_index_store_price_help'];?>，且不能高于市场价。<br>
            此价格为煤炭实际销售价格，如果煤炭存在规格，该价格显示最低价格。</p>
        </dd>
      </dl>
      <dl>
      ...
      <dl>
        ...
        <dd>
          ...
          <p class="hint">价格必须是0.00~9999999之间的数字，此价格为商户对所销售的煤炭实际成本价格进行备注记录，非必填选项，不会在前台销售页面中显示。</p>
        </dd>
      </dl>
      <dl>
      ...
      <dl nc_type="spec_group_dl_<?php echo $i;?>" nctype="spec_group_dl" class="spec-bg" <?php if($k == '1'){?>spec_img="t"<?php }?>>
        ...
        <dd <?php if($k == '1'){?>nctype="sp_group_val"<?php }?>>
          ...
          <p class="hint">添加或取消颜色规格时，提交后请编辑图片以确保煤炭图片能够准确显示。</p>
          <?php }?>
        </dd>
      </dl>
      <?php $i++;?>
      ...
      <dl>
        ...
        <dd>
          ...
          <span></span>
          <p class="hint">设置最低库存预警值。当库存低于预警值时企业中心煤炭列表页库存列红字提醒。<br>
            请填写0~255的数字，0为不预警。</p>
        </dd>
      </dl>
      <dl>
            ...
            <div class="ncsc-brand-select-container">
              <div class="brand-index" data-tid="<?php echo $output['goods_class']['type_id'];?>" data-url="<?php echo urlShop('store_goods_add', 'ajax_get_brand');?>">
                <div class="letter" nctype="letter">
                  <ul>
                    <li><a href="javascript:void(0);" data-letter="all">全部煤炭企业</a></li>
                ...
                <div class="search" nctype="search">
                  <input name="search_brand_keyword" id="search_brand_keyword" type="text" class="text" placeholder="企业名称关键字查找"/><a href="javascript:void(0);" class="ncsc-btn-mini" style="vertical-align: top;">Go</a></div>
              </div>
              ...
              <div class="no-result" nctype="noBrandList" style="display: none;">没有符合"<strong>搜索关键字</strong>"条件的煤炭企业</div>
              <div class="tc"><a href="javascript:void(0);" class="ncsc-btn-mini" onclick="$(this).parents('.ncsc-brand-select-container:first').hide();">关闭煤炭企业列表</a></div>
            </div>
          </div>
        </dd>
      </dl>
      ...
      <?php /*?><h3 id="demo3">特殊煤炭</h3>
      <!-- 只有可发布虚拟煤炭才会显示 S -->
      <?php if ($output['goods_class']['gc_virtual'] == 1) {?>
      <dl class="special-01">
        <dt>虚拟煤炭<?php echo $lang['nc_colon'];?></dt>
        <dd>
          ...
          <p class="hint vital">*虚拟煤炭不能参加限时折扣和组合销售两种促销活动。也不能赠送赠品和推荐搭配。</p>
        </dd>
      </dl>
      <dl class="special-01" nctype="virtual_valid" <?php if ($output['goods']['is_virtual'] == 0) {?>style="display:none;"<?php }?>>
        <dt><i class="required">*</i>虚拟煤炭有效期至<?php echo $lang['nc_colon'];?></dt>
        <dd>
          ...
          <span></span>
          <p class="hint">虚拟煤炭可兑换的有效期，过期后煤炭不能购买，电子兑换码不能使用。</p>
        </dd>
      </dl>
      <dl class="special-01" nctype="virtual_valid" <?php if ($output['goods']['is_virtual'] == 0) {?>style="display:none;"<?php }?>>
        <dt><i class="required">*</i>虚拟煤炭购买上限<?php echo $lang['nc_colon'];?></dt>
        <dd>
          ...
          <span></span>
          <p class="hint">请填写1~10之间的数字，虚拟煤炭最高购买数量（吨）不能超过10个。</p>
        </dd>
      </dl>
      <dl class="special-01" nctype="virtual_valid" <?php if ($output['goods']['is_virtual'] == 0) {?>style="display:none;"<?php }?>>
      ...
      <dl class="special-02" nctype="virtual_null" <?php if ($output['goods']['is_virtual'] == 1) {?>style="display:none;"<?php }?>>
        <dt>F码煤炭<?php echo $lang['nc_colon'];?></dt>
        <dd>
          <ul class="ncsc-form-radio-list">
            <li>
          ...
          <p class="hint vital">*F码煤炭不能参加团购、限时折扣和组合销售三种促销活动。也不能预售和推荐搭配。</p>
        </dd>
      </dl>
      <dl class="special-02" nctype="fcode_valid" <?php if ($output['goods']['is_fcode'] == 0) {?>style="display:none;"<?php }?>>
        ...
        <dd>
          ...
          <span></span>
          <p class="hint">请填写100以内的数字。编辑煤炭时添加F码会不计算原有F码数量继续生成相应数量。</p>
        </dd>
      </dl>
      <dl class="special-02" nctype="fcode_valid" <?php if ($output['goods']['is_fcode'] == 0) {?>style="display:none;"<?php }?>>
        <dt>
      ...
      <dl class="special-03" nctype="virtual_null" <?php if ($output['goods']['is_virtual'] == 1) {?>style="display:none;"<?php }?>>
        ...
        <dd>
          ...
          <p class="hint vital">*预售煤炭不能参加团购、限时折扣和组合销售三种促销活动。也不能推荐搭配。</p>
        </dd>
      </dl>
      ...
      <dl nctype="virtual_null" <?php if ($output['goods']['is_virtual'] == 1) {?>style="display:none;"<?php }?>>
        ...
        <dd>
          ...
          <p class="hint">运费设置为 0 元，前台煤炭将显示为免运费。</p>
        </dd>
      </dl>
      <!-- 煤炭物流信息 E -->
      ...
      <dl>
        ...
        <dd>
          ...
          <p class="hint">只有库存为零的煤炭才可以设为预约煤炭。企业补货后，平台自动发布消息通知已经预约的会员。</p>
        </dd>
      </dl>
    ...
    <div class="bottom tc hr32">
      <label class="submit-border">
        <input type="submit" class="submit" value="<?php if ($output['edit_goods_sign']) {echo '提交';} else {?><?php echo $lang['store_goods_add_next'];?>，上传煤炭图片<?php }?>" />
      </label>
    </div>
  </form>
</div>
...
$(function(){
    ...
    }, '<i class="icon-exclamation-sign"></i>煤炭价格不能高于市场价格');
    ...
    $('#goods_form').validate({
        ...
        messages : {
            ...
            g_jingle : {
                maxlength   : '<i class="icon-exclamation-sign"></i>煤炭卖点不能超过140个字符'
            },
            g_price : {
            ...
            image_path : {
                required    : '<i class="icon-exclamation-sign"></i>请设置煤炭主图'
            },
            g_vindate : {
