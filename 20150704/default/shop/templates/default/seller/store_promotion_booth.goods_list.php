...
<?php if ($output['isOwnShop']) { ?>
  <a class="ncsc-btn ncsc-btn-green" href="javascript:void(0);" nctype="select_goods"><i class="icon-plus-sign"></i>添加煤炭</a>
...
<?php } else { ?>
  ...
  <a class="ncsc-btn ncsc-btn-green" href="javascript:void(0);" nctype="select_goods" style="right:100px"><i class="icon-plus-sign"></i>添加煤炭</a>
  <?php } ?>
...
<div class="alert alert-block mt10">
  <ul>
    <li>1、被推荐煤炭将在该煤炭所在的分类及其上级分类的煤炭列表左侧随机出现。</li>
    <?php if (C('promotion_booth_goods_sum') != 0) {?>
    <li>2、您最多可以推荐<?php echo C('promotion_booth_goods_sum');?>个煤炭。</li>
    <?php }?>
  </ul>
</div>
<?php } else { ?>
...
<?php }?>
  <ul>
    ...
    <li>2、<strong style="color: red">相关费用会在煤企的账期结算中扣除</strong>。</li>
    <li>3、被推荐煤炭将在该煤炭所在的分类及其上级分类的煤炭列表左侧随机出现。</li>
    <?php if (C('promotion_booth_goods_sum') != 0) {?>
    <li>4、您最多可以推荐<?php echo C('promotion_booth_goods_sum');?>个煤炭。</li>
    <?php }?>
  </ul>
</div>
<?php } ?>
...
<div nvtype="div_goods_select" class="div-goods-select" style="display: none;">
    <table class="search-form">
      <tr><th class="w150"><strong>第一步：搜索店内煤炭</strong></th><td class="w160"><input nctype="search_goods_name" type="text w150" class="text" name="goods_name" value=""/></td>
        <td class="w70 tc"><label class="submit-border"><input nctype="btn_search_goods" type="button" value="<?php echo $lang['nc_search'];?>" class="submit"/></label></td><td class="w10"></td><td><p class="hint">不输入名称直接搜索将显示店内所有出售中的煤炭</p></td>
      </tr>
    </table>
...
<table class="ncsc-default-table">
  ...
  <tbody nctype="choose_goods_list">
    <tr nctype="tr_no_promotion" style="display:none;">
      <td colspan="20" class="norecord"><div class="no-promotion"><i class="zw"></i><span>推荐展位列表暂无内容，请选择添加平台推荐展位煤炭。</span></div></td>
    </tr>
