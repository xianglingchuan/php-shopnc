  ...
  <div class="div-goods-select">
    <form method="GET">
      ...
      <table class="search-form">
        <tr>
          <th class="w250"><strong>选择参加活动的煤炭，勾选并提交平台审核</strong></th>
          <td class="w160"><input type="text" class="text w150" name="name" value="<?php echo $output['search']['name'];?>" placeholder="搜索煤炭名称"/></td>
    ...
    <form method="POST" id="apply_form" onsubmit="ajaxpost('apply_form','','','onerror');" action="index.php?act=store_activity&op=activity_apply_save">
      ...
      <?php }else{?>
      <div class="warning-option"><i class="icon-warning-sign"></i><span><?php echo $lang['no_record'];//您尚未发布任何煤炭?></span></div>
      <?php }?>
      <?php if(!empty($output['goods_list']) and is_array($output['goods_list'])){?>
