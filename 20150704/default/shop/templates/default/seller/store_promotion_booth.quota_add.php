...
$(document).ready(function(){
	...
	$("#add_form").validate({
		...
		messages : {
			booth_quota_quantity : {
				required : '<i class="icon-exclamation-sign"></i>套餐购买数量（吨）不能为空，且必须为1~12之间的整数',
				digits : '<i class="icon-exclamation-sign"></i>套餐购买数量（吨）不能为空，且必须为1~12之间的整数',
				range : '<i class="icon-exclamation-sign"></i>套餐购买数量（吨）不能为空，且必须为1~12之间的整数'
			}
		}
	});
});
