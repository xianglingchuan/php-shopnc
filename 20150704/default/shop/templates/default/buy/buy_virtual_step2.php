  ...
  <input type="hidden" name="quantity" value="<?php echo $output['goods_info']['quantity'];?>">
    <div class="ncc-receipt-info">
      ...
      <div id="invoice_list" class="ncc-candidate-items">
        ...
        <p><i class="icon-info-sign"></i>您本次购买的煤炭不需要收货地址，请正确输入接收手机号码，确保及时获得“电子兑换码”。可使用您已经绑定的手机或重新输入其它手机号码。</p>
      </div>
    </div><div class="ncc-receipt-info"><div class="ncc-receipt-info-title">
      <h3>虚拟服务类煤炭清单</h3><a href="index.php?act=buy_virtual&op=buy_step1&goods_id=<?php echo $_POST['goods_id'];?>&quantity=<?php echo $_POST['quantity'];?>">返回上一步</a></div>
    <table class="ncc-table-style" nc_type="table_cart">
      ...
      <tbody>
        <tr>
          <th colspan="20">煤企：<a href="<?php echo urlShop('show_store','index',array('store_id'=>$output['store_info']['store_id']));?>"><?php echo $output['store_info']['store_name'];?></a> <span member_id="<?php echo $output['store_info']['member_id'];?>"></span>
          </th>
        </tr>
    ...
    <!-- E voucher list -->
      ...
      <tfoot>
        <tr>
          <td colspan="20"><div class="ncc-all-account">煤炭总价<em id="orderTotal"><?php echo $output['goods_info']['goods_total']; ?></em>元</div></td>
        </tr>
      </tfoot>
    </table>
