...
class store_goods_addControl extends BaseSellerControl {
    ...
    public function add_step_twoOp() {
        ...
        // 如果不是自营煤企或者自营煤企未绑定全部煤炭类目，读取绑定分类
    ...
    public function save_goodsOp() {
        if (chksubmit()) {
            ...
            $common_array['goods_state']        = ($this->store_info['store_state'] != 1) ? 0 : intval($_POST['g_state']);            // 煤企关闭时，煤炭下架
            ...
            $common_array['is_appoint']         = intval($_POST['is_appoint']);     // 只有库存为零的煤炭可以预约
            $common_array['appoint_satedate']   = $common_array['is_appoint'] == 1 ? strtotime($_POST['g_saledate']) : '';   // 预约煤炭的销售时间
            $common_array['is_presell']         = $common_array['goods_state'] == 1 ? intval($_POST['is_presell']) : 0;     // 只有出售中的煤炭可以预售
            $common_array['presell_deliverdate']= $common_array['is_presell'] == 1? strtotime($_POST['g_deliverdate']) : ''; // 预售煤炭的发货时间
            ...
            if ($common_id) {
                ...
                // 记录日志
                $this->recordSellerLog('添加煤炭，平台货号:'.$common_id);
