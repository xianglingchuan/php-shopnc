...
class store_promotion_boothControl extends BaseSellerControl {
    ...
    public function choosed_goodsOp() {
        ...
        if (count($bootgoods_info) >= C('promotion_booth_goods_sum')) {
            $data = array('result' => 'false', 'msg' => '只能添加'.C('promotion_booth_goods_sum').'个煤炭');
            $this->_echoJson($data);
        }
        // 煤炭是否已经被添加
        $bootgoods_info = array_under_reset($bootgoods_info, 'goods_id');
        if (isset($bootgoods_info[$gid])) {
            $data = array('result' => 'false', 'msg' => '煤炭已经添加，请选择其他煤炭');
            $this->_echoJson($data);
        }
        ...
        $this->recordSellerLog('添加推荐展位煤炭，煤炭id：'.$goods_info['goods_id']);
    ...
    public function del_choosed_goodsOp() {
        ...
        if ($result) {
            $this->recordSellerLog('删除推荐展位煤炭，煤炭id：'.$gid);
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'booth_goods_list':
                $menu_array	= array(
                    1=>array('menu_key'=>'booth_goods_list', 'menu_name'=>'煤炭列表', 'menu_url'=>urlShop('store_promotion_booth', 'booth_goods_list'))
                );
                break;
