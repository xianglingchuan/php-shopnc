              ...
              <div class="ncsc-brand-select-container">
                <div class="brand-index" data-url="index.php?act=common&op=ajax_get_brand">
                  ...
                  <div class="search" nctype="search">
                    <input name="search_brand_keyword" id="search_brand_keyword" type="text" class="text" placeholder="企业名称关键字查找"/>
        ...
        <tr class="hover edit">
          ...
          <td><dl class="goods-info">
              ...
              <dd class="goods-type">
                ...
                <span class="virtual" title="虚拟兑换煤炭">虚拟</span>
                <?php }?>
                <?php if ($v['is_fcode'] ==1) {?>
                <span class="fcode" title="F码优先购买煤炭">F码</span>
                <?php }?>
                <?php if ($v['is_presell'] ==1) {?>
                <span class="presell" title="预先发售煤炭">预售</span>
                <?php }?>
                <?php if ($v['is_appoint'] ==1) {?>
                <span class="appoint" title="预约销售提示煤炭">预约</span>
                <?php }?>
                <i class="icon-tablet <?php if ($v['mobile_body'] != '') {?>open<?php }?>" title="手机端煤炭详情"></i> </dd>
          ...
          <td class><p><?php echo $v['gc_name'];?></p>
            <p class="goods-brand">煤炭企业：<?php echo $v['brand_name'];?></p></td>
...
function goods_verify(ids) {
    ...
    CUR_DIALOG = ajax_form('goods_verify', '审核煤炭', _uri, 350);
}
</script>
