...
CREATE TABLE `#__circle` (
  ...
  `circle_gcount` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '圈子煤炭数',
...
CREATE TABLE `#__circle_theme` (
  ...
  `has_goods` tinyint(3) unsigned NOT NULL DEFAULT '0' COMMENT '煤炭标记 1是 0否',
...
CREATE TABLE `#__circle_thg` (
  `themegoods_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '主题煤炭id',
  `theme_id` int(11) NOT NULL COMMENT '主题id',
  `reply_id` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '回复id',
  `circle_id` int(11) unsigned NOT NULL COMMENT '圈子id',
  `goods_id` int(11) NOT NULL COMMENT '煤炭id',
  `goods_name` varchar(100) NOT NULL COMMENT '煤炭名称',
  `goods_price` decimal(10,2) NOT NULL COMMENT '煤炭价格',
  `goods_image` varchar(1000) NOT NULL COMMENT '煤炭图片',
  `store_id` int(11) NOT NULL COMMENT '店铺id',
  `thg_type` tinyint(4) NOT NULL DEFAULT '0' COMMENT '煤炭类型 0为本商城、1为淘宝 默认为0',
  `thg_url` varchar(1000) DEFAULT NULL COMMENT '煤炭链接',
  PRIMARY KEY (`themegoods_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='主题煤炭表';
...
('circle_seokeywords', '圈子,帮派,讨论小组,购物,生活,分享,show,秀,煤炭,电子商务,社区,消费者社区,论坛,资讯,热门话题,朋友'),
