...
//         $this->_add_points();
        ...
        if (C('cache_open')){//如果浏览记录已经存入了缓存中，则将其整理到数据库中
            ...
            for ($i=0; $i<$countnum; $i+=$eachnum){//每次查询100条
                ...
                foreach ((array)$member_list as $k=>$v){
                    ...
                    if ($browse_goodsid) {
                        //删除缓存中多余的浏览历史记录，仅保留最近的30条浏览历史，先取出最近30条浏览历史的煤炭ID
    ...
    private function _member_stat(){
        ...
        for ($stime = $start_time; $stime < time(); $stime = $stime+86400){
            ...
            //统计订单下单量和下单金额
            $field = ' order.order_id,add_time,buyer_id,buyer_name,order_amount,log_orderstate,payment_code';
            $where = array();
            $where['order.order_state'] = array(array('neq',ORDER_STATE_NEW), array('neq',ORDER_STATE_CANCEL),'and');//去除未支付和已取消订单
            ...
            foreach ((array)$update_arr as $k=>$v){
                foreach ($v as $m_k=>$m_v){
                    ...
                    $where = array();
                    $where['order.order_state'] = array(array('neq',ORDER_STATE_NEW), array('neq',ORDER_STATE_CANCEL),'and');//去除未支付和已取消订单
                    $where['order.add_time'] = array('between',array($up_stime,$up_etime));
                    //货到付款当交易成功进入统计，非货到付款当付款后进入统计
                    $where['order.payment_code'] = array('exp',"(order.payment_code='offline' and order.order_state = '".ORDER_STATE_SUCCESS."') or (order.payment_code<>'offline')");
                    $ordergoods_amount = $model->statByOrderGoods($where, 'SUM(goods_num) as gnum');
