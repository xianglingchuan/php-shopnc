...
class stat_industryControl extends SystemControl{
	...
	public function rankOp(){
		...
		//得到统计图数据
		$goods_stat_arr['series'][0]['name'] = '下单煤炭数';
		$goods_stat_arr['title'] = "行业煤炭50强";
		$goods_stat_arr['legend']['enabled'] = false;
        $goods_stat_arr['yAxis'] = '下单煤炭数';
    ...
    public function priceOp(){
		...
		if ($pricerange_arr){
		    $goodsnum_stat_arr['series'][0]['name'] = '下单煤炭数';
			...
			$goodsnum_stat_arr['title'] = '行业价格下单煤炭数';
	...
	public function general_listOp(){
	    ...
	    if ($gc_childarr){
    		...
    		foreach ($gc_childarr as $k=>$v){
    		    ...
    		    $tmp['unordergcount'] = intval($goods_list[$v['gc_id']]['goodscount']) - intval($ordergoods_list[$v['gc_id']]['ordergcount']);//计算无销量煤炭数
    		    $statlist_tmp[]= $tmp;
    		}
    		$statlist = array();
    		//整理排序
            ...
            if ($this->search_arr['exporttype'] == 'excel'){
                ...
                $statheader[] = array('text'=>'有销量煤炭数','key'=>'ordergcount','isorder'=>1);
                $statheader[] = array('text'=>'销量','key'=>'ordergnum','isorder'=>1);
                $statheader[] = array('text'=>'销售额（元）','key'=>'orderamount','isorder'=>1);
                $statheader[] = array('text'=>'煤炭总数','key'=>'goodscount','isorder'=>1);
                $statheader[] = array('text'=>'无销量煤炭数','key'=>'unordergcount','isorder'=>1);
                //导出Excel
		...
		$statheader = array();
        $statheader[] = array('text'=>'类目名称','key'=>'gc_name');
        $statheader[] = array('text'=>'<span title="类目下所有煤炭的平均单价" class="tip icon-question-sign"></span>&nbsp;平均价格（元）','key'=>'priceavg','isorder'=>1);
        $statheader[] = array('text'=>'<span title="类目下从昨天开始最近30天有效订单中有销量的煤炭总数" class="tip icon-question-sign"></span>&nbsp;有销量煤炭数','key'=>'ordergcount','isorder'=>1);
        $statheader[] = array('text'=>'<span title="类目下从昨天开始最近30天有效订单中煤炭总售出件数" class="tip icon-question-sign"></span>&nbsp;销量','key'=>'ordergnum','isorder'=>1);
        $statheader[] = array('text'=>'<span title="类目下从昨天开始最近30天有效订单中煤炭总销售额" class="tip icon-question-sign"></span>&nbsp;销售额（元）','key'=>'orderamount','isorder'=>1);
        $statheader[] = array('text'=>'<span title="类目下所有煤炭的数量" class="tip icon-question-sign"></span>&nbsp;煤炭总数','key'=>'goodscount','isorder'=>1);
        $statheader[] = array('text'=>'<span title="类目下从昨天开始最近30天无销量的煤炭总数" class="tip icon-question-sign"></span>&nbsp;无销量煤炭数','key'=>'unordergcount','isorder'=>1);
