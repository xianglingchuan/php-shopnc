...
class stat_goodsControl extends SystemControl{
    ...
    public function goods_saleOp(){
        ...
        if ($_GET['exporttype'] == 'excel'){
			...
			//header
			$excel_data[0][] = array('styleid'=>'s_title','data'=>'煤炭名称');
			$excel_data[0][] = array('styleid'=>'s_title','data'=>'平台货号');
			$excel_data[0][] = array('styleid'=>'s_title','data'=>'店铺名称');
			$excel_data[0][] = array('styleid'=>'s_title','data'=>'下单煤炭件数');
		    ...
		    $excel_obj->addWorksheet($excel_obj->charset('煤炭销售明细',CHARSET));
		    $excel_obj->generateXML($excel_obj->charset('煤炭销售明细',CHARSET).date('Y-m-d-H',time()));
			exit();
		} else {
			//查询企业
