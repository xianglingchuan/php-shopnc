...
class stat_marketingControl extends SystemControl{
	...
	public function promotionOp(){
		...
		if ($statlist_tmp){
    		...
    		$stat_json['goodsnum'] = getStatData_Pie(array('title'=>'下单煤炭数','name'=>'下煤炭数(%)','label_show'=>false,'series'=>$stat_arr['goodsnum']));
	...
	public function groupgoodsOp(){
		...
		$statheader = array();
        $statheader[] = array('text'=>'煤炭名称','key'=>'goods_name','class'=>'alignleft');
        $statheader[] = array('text'=>'下单煤炭数','key'=>'goodsnum','isorder'=>1);
        $statheader[] = array('text'=>'下单金额','key'=>'goodsamount','isorder'=>1);
        $statheader[] = array('text'=>'取消煤炭数','key'=>'cancelgoodsnum','isorder'=>1);
        $statheader[] = array('text'=>'取消金额','key'=>'cancelgoodsamount','isorder'=>1);
        $statheader[] = array('text'=>'完成煤炭数','key'=>'finishgoodsnum','isorder'=>1);
	    ...
	    if ($this->search_arr['exporttype'] == 'excel'){
		    ...
		    $excel_obj->addWorksheet($excel_obj->charset('团购煤炭统计',CHARSET));
		    $excel_obj->generateXML($excel_obj->charset('团购煤炭统计',CHARSET).date('Y-m-d-H',time()));
			exit();
        } else {
