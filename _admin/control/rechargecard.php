...
class rechargecardControl extends SystemControl
{
    ...
    public function log_listOp()
    {
        ...
        $condition = $this->getLogRequestCondition();
        $list_log = $model->getRechargeCardBalanceLogList($condition);
    ...
    private function getLogRequestCondition()
    {
        $condition = array();

        $mname = trim($_GET['mname']);
        if ($mname) {
            $condition['member_name'] = $mname;
        }

        $stime = $_GET['stime'] ? strtotime($_GET['stime'] . ' 00:00:00') : 0;
        $etime = $_GET['etime'] ? strtotime($_GET['etime'] . ' 00:00:00') : 0;
        if ($stime > 0 || $etime > 0) {
    ...
    public function log_export_step1Op()
    {
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){
                ...
                //如果数量小，直接下载
                $data = $model->getRechargeCardBalanceLogList($condition, self::EXPORT_SIZE);
            ...
            $data = $model->getRechargeCardBalanceLogList($condition, 20, "{$limit1},{$limit2}");
