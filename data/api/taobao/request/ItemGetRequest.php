...
{
	/** 
	 * 需要返回的煤炭对象字段。可选值：Item煤炭结构体中所有字段均可返回；多个字段用“,”分隔。如果想返回整个子对象，那字段为item_img，如果是想返回子对象里面的字段，那字段为item_img.url。新增返回字段：second_kill（是否秒杀煤炭）、auto_fill（代充煤炭类型）,props_name（煤炭属性名称）。新增返回字段：item_weight(煤炭的重量，格式为数字，包含小数)、item_size(煤炭的体积，格式为数字，包含小数)
	 **/
	private $fields;
