...
class buy_virtualLogic {
    ...
    /**
     * 虚拟煤炭购买第一步，得到购买数据(煤炭、店铺、会员)
     * @param int $goods_id 煤炭ID
    ...
    public function getBuyStepData($goods_id, $quantity, $member_id) {
        ...
        if (empty($goods_info)) {
            return callback(false, '该煤炭不符合购买条件，可能的原因有：下架、不存在、过期等');
        }
...
//        }
    ...
    /**
     * 虚拟煤炭购买第三步
     * @param array $post 接收POST数据，必须传入goods_id:煤炭ID，quantity:购买数量,buyer_phone:接收手机,buyer_msg:买家留言
     * @param int $member_id
     * @return array
     */
