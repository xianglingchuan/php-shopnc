...
class buyLogic {
    ...
    public function buyStep1($cart_id, $ifcart, $member_id, $store_id) {
        ...
        //得到页面所需要数据：收货地址、发票、代金券、预存款、煤炭列表等信息
    ...
    public function getCartList($cart_id, $member_id) {
        ...
        if (empty($buy_items)) {
            return callback(false, '所购煤炭无效');
        }

        if (count($buy_items) > 50) {
            return callback(false, '一次最多只可购买50种煤炭');
        }
    ...
    public function getGoodsList($cart_id, $member_id, $store_id) {
        ...
        if(empty($goods_info)) {
            return callback(false, '煤炭已下架或不存在');
        }

        //不能购买自己店铺的煤炭
        if ($goods_info['store_id'] == $store_id) {
            return callback(false, '不能购买自己店铺的煤炭');
        }
    ...
    /**
     * 购买第一步：返回煤炭、促销、地址、发票等信息，然后交前台抛出
     * @param unknown $member_id
     * @param unknown $data 煤炭信息
     * @return
     */
    public function getBuyStep1Data($member_id, $data) {
        ...
        //将煤炭ID、数量、运费模板、运费序列化，加密，输出到模板，选择地区AJAX计算运费时作为参数使用
        ...
        if (!empty($pay_goods_list['offline'])) {
            ...
            //如果所购煤炭只支持线上支付，支付方式不允许修改
            $result['deny_edit_payment'] = true;
        }

        //发票 :只有所有煤炭都支持增值税发票才提供增值税发票
     ...
     * 如果店铺未设置满免规则，且使用运费模板，按运费模板计算，如果其中有煤炭使用相同的运费模板，则两种煤炭数量相加后再应用该运费模板计算（即作为一种煤炭算运费）
     * 如果未找到运费模板，按免运费处理
     * 如果没有使用运费模板，煤炭运费按快递价格计算，运费不随购买数量增加
     */
...
//         $store_gc_id_commis_rate_list = Model('store_bind_class')->getStoreGcidCommisRateList($goods_list);
        ...
        if($append_premiums_to_cart_list === false) {
            throw new Exception('抱歉，您购买的煤炭库存不足，请重购买');
        } else {
...
//         $this->_order_data['store_gc_id_commis_rate_list'] = $store_gc_id_commis_rate_list;
    ...
    private function _createOrderStep4() {
        ...
        foreach ($store_cart_list as $store_id => $goods_list) {
            ...
            //取得本店优惠额度(后面用来计算每件煤炭实际支付金额，结算需要)
            ...
            foreach ($goods_list as $goods_info) {
                if (!$goods_info['state'] || !$goods_info['storage_state']) {
                    throw new Exception('部分煤炭已经下架或库存不足，请重新选择');
                }
                if (!intval($goods_info['bl_id'])) {
                    ...
                    foreach ($goods_info['bl_goods_list'] as $bl_goods_info) {
                        ...
                        //计算煤炭实际支付金额(goods_price减去分摊优惠金额后的值)
            ...
            //将因舍出小数部分出现的差值补到最后一个煤炭的实际成交价中(煤炭goods_price=0时不给补，可能是赠品)
            ...
            if (!$insert) {
                throw new Exception('订单保存失败[未生成煤炭数据]');
            }
