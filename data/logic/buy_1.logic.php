...
class buy_1Logic {
    ...
    /**
     * 煤炭金额计算(分别对每个煤炭/优惠套装小计、每个店铺小计)
     * @param unknown $store_cart_list 以店铺ID分组的购物车煤炭信息
     * @return array
     */
    ...
    /**
     * 取得店铺级优惠 - 跟据煤炭金额返回每个店铺当前符合的一条活动规则，如果有赠品，则自动追加到购买列表，价格为0
     * @param unknown $store_goods_total 每个店铺的煤炭金额小计，以店铺ID为下标
	...
	/**
	 * 重新计算每个店铺最终煤炭总金额(最初计算金额减去各种优惠/加运费)
	 * @param array $store_goods_total 店铺煤炭总金额
	 * @param array $preferential_array 店铺优惠活动内容
	 * @param string $preferential_type 优惠类型，目前只有一个 'mansong'
	 * @return array 返回扣除优惠后的店铺煤炭总金额
	 */
	...
	/**
	 * 取得店铺可用的代金券
	 * @param array $store_goods_total array(店铺ID=>煤炭总金额)
	 * @return array
	 */
	 ...
	 * @param array $store_goods_total (店铺ID=>煤炭总金额)
	 * @return array
	 */
    ...
    /**
     * 判断煤炭是不是限时折扣中，如果购买数量若>=规定的下限，按折扣价格计算,否则按原价计算
    ...
    /**
     * 输出有货到付款时，在线支付和货到付款及每种支付下煤炭数量和详细列表
     * @param $buy_list 煤炭列表
     * @return 返回 以支付方式为下标分组的煤炭列表
     */
    public function getOfflineGoodsPay($buy_list) {
        ...
        if ($offline_pay) {
            //下单里包括平台自营煤炭并且平台已开启货到付款，则显示货到付款项及对应煤炭数量,取出支持货到付款的店铺ID组成的数组，目前就一个，DEFAULT_PLATFORM_STORE_ID
     ...
     * @param array $store_goods_total 最初店铺煤炭总金额
     * @param array $store_final_goods_total 去除各种店铺级促销后，最终店铺煤炭总金额(不含运费)
     * @return array
     */
     ...
     * @param array $store_goods_total 每个店铺的煤炭金额小计，以店铺ID为下标
     * @return array
     */
    ...
    /**
     * 取得店铺运费(使用运费模板的煤炭运费不会计算，但会返回模板信息)
     * 先将免运费的店铺运费置0，然后算出店铺里没使用运费模板的煤炭运费之和 ，存到iscalced下标中
     * 然后再计算使用运费模板的信息(array(店铺ID=>array(运费模板ID=>购买数量))，放到nocalced下标里
     * @param array $buy_list 购买煤炭列表
    ...
    public function getStoreFreightList($buy_list = array(), $free_freight_sid_list) {
        ...
        //然后算出店铺里没使用运费模板(优惠套装煤炭除外)的煤炭运费之和(格式:店铺ID=>运费)
        //定义数组，存放店铺优惠套装煤炭运费总额 store_id=>运费
        ...
        //最后再计算使用运费模板的信息(店铺ID，运费模板ID，购买数量),使用使用相同运费模板的煤炭数量累加
        $freight_list = array();
...
//         if (empty($pd_payment_info)) return;
	 ...
	 * 更新库存与销量
	 *
	 * @param array $buy_items 煤炭ID => 购买数量
	 */
    ...
    /**
     * 直接购买时返回最新的在售煤炭信息（需要在售）
     *
     * @param int $goods_id 所购煤炭ID
    ...
    private function _getGoodsOnlineInfo($goods_id,$quantity) {
        ...
        //cart_id=goods_id,优惠套装目前只能进购物车,不能现货购买
    ...
    /**
     * 直接购买时，判断煤炭是不是正在团购中，如果是，按团购价格计算，购买数量若超过团购规定的上限，则按团购上限计算
     * @param array $goods_info
     */
    ...
    /**
     * 批量判断购物车内的煤炭是不是限时折扣中，如果购买数量若>=规定的下限，按折扣价格计算,否则按原价计算
     * 并标识该煤炭为限时煤炭
     * @param array $cart_list
     */
    ...
    private function _getBundlingCartList(& $cart_list) {
        ...
        foreach ($cart_list as $key => $cart_info) {
            ...
            //使用最新的煤炭名称、图片,如果一旦有煤炭下架，则整个套装置置为无效状态
            $total_down_price = 0;
