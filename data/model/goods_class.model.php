...
class goods_classModel extends Model
{
    ...
    public function delGoodsClassByGcIdString($gcids) {
        ...
        // 煤炭下架
        Model('goods')->editProducesLockUp(array('goods_stateremark' => '煤炭分类被删除，需要重新选择分类'), array('gc_id' => array('in', $gcid_array)));
        return true;
    }
    ...
    public function get_all_category($update_all = 0) {
        ...
        if ($update_all == 1 || !($gc_list = rkcache('all_categories'))) {
            ...
            if (is_array($class_list) && !empty($class_list)) {
                ...
                $type_brands = $this->get_type_brands($type_ids);//类型关联企业
                foreach ($gc_list as $key => $value) {
                    ...
                    if (is_array($class2s) && !empty($class2s)) {//第2级关联企业
