...
class refund_returnModel extends Model{
		...
		if ($refund_id > 0) {
    	    ...
    	    try {
    			...
    			if ($state && $order_state == $order_paid) {
            	    Logic('order')->changeOrderStateCancel($order, 'system', '系统', '煤炭全部退款完成取消订单',false);
            	}
    			if ($state) {
    			    $order_array = array();
    			    $order_amount = $order['order_amount'];//订单金额
	...
	public function getGoodsRefundList($order_list = array(), $order_refund = 0) {
	    ...
	    if (!empty($order_list) && is_array($order_list)) {
    	    foreach ($order_list as $key => $value) {
    	        $order_id = $key;
    	        $goods_list = $value['extend_order_goods'];//订单煤炭
        	    ...
        	    } elseif ($order_state > $order_paid && !empty($goods_list) && is_array($goods_list)) {//已发货后对煤炭操作
        	        $refund = $this->getRefundState($value);//根据订单状态判断是否可以退款退货
            	    foreach ($goods_list as $k => $v) {
            	        $goods_id = $v['rec_id'];//订单煤炭表编号
	...
	public function getComplainRefundList($order, $order_goods_id = 0) {
	    $list = array();
	    $refund_list = array();//已退或处理中煤炭
	    $refund_goods = array();//可退煤炭
	    if (!empty($order) && is_array($order)) {
            ...
            foreach ($goods_list as $k => $v) {
                ...
                if ($v['refund_state'] > 2) {//可退煤炭
                    $goods_pay_price = $v['goods_pay_price'];//煤炭实际成交价
	...
	public function getRightOrderList($order_condition, $order_goods_id = 0){
		...
		$condition['order_id'] = $order_id;
		if ($order_goods_id > 0) {
		    $condition['rec_id'] = $order_goods_id;//订单煤炭表编号
        }
